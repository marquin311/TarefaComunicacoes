...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  int index;
  if (c >= '0' && c <= '9') {
       index = 1 + (c - '0'); // dígitos começam em índice 1
  } else if (c >= 'A' && c <= 'Z') {
       index = 1 + 10 + (c - 'A'); // após os dígitos
  } else if (c >= 'a' && c <= 'z') {
       index = 1 + 10 + 26 + (c - 'a'); // após os maiúsculos
  } else {
       index = 0; // caractere desconhecido: desenha "Nothing"
  }
  // Agora, desenhe o caractere usando o índice calculado.
  // Essa parte depende de como a sua função original trabalha.
  ssd1306_draw_char_from_font(ssd, font, index, x, y);
...
void ssd1306_draw_char_from_font(ssd1306_t *ssd, const uint8_t *font, int index, uint8_t x, uint8_t y) {
  for (uint8_t i = 0; i < 8; i++) {  
      uint8_t line = font[index * 8 + i];
      for (uint8_t j = 0; j < 8; j++) {  
          if (line & (1 << j)) { // Troca a ordem dos bits para corrigir a inversão
